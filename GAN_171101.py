...
def CheckAccuracy( x, g_z, data_cols, label_cols=[], seed=0, with_class=False, data_dim=2 ):
    ...
    xgb_params = {
        # 'tree_method': 'hist', # for fast evaluation
        'max_depth': 4, # for fast evaluation
...
#             d_l_r = discriminator_model.train_on_batch(x, classes)
            ...
            # d_l_r = discriminator_model.train_on_batch(x, np.ones(batch_size))
            # d_l_g = discriminator_model.train_on_batch(g_z, np.zeros(batch_size))
